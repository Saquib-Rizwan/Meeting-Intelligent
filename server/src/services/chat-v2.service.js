...
const buildStructuredAnswer = ({ summary, sources, queryTokens, meetingCount, chunkCount }) => {
  ...
  return [
    `Summary: ${summary}`,
    `Coverage: Based on ${meetingCount} meeting${meetingCount === 1 ? "" : "s"} and ${chunkCount} transcript segment${chunkCount === 1 ? "" : "s"}.`,
...
export const chatService = {
  async answerQuestion({ question, meetingId, meetingIds }) {
      ...
      if (hasAnswer) {
        ...
        const result = {
          answer: buildStructuredAnswer({
            summary: rawAnswer || crossChunkSummary,
            sources,
            queryTokens,
            meetingCount,
            chunkCount
    ...
    const result = {
      answer: buildStructuredAnswer({
        summary: crossChunkSummary || fallbackSummary,
        sources,
        queryTokens,
        meetingCount,
        chunkCount
